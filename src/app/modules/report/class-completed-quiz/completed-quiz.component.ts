...
@Component({
  selector: 'app-class-completed-quiz',
  templateUrl: './completed-quiz.component.html',
  styleUrls: ['./completed-quiz.component.less'],
})
export class ClassCompletedQuizComponent implements OnInit {
  dataloading = false;
  searchValue = '';
  total = 1;
  data: Array<any> = [];
  ...
  async loadDataFromServer(paging: IMultiPaging) {
    ...
    this.studentprogresses$ = this.reportService
      .getClassProgressData(paging, this.online)
      .pipe(
        catchError((x: any) => []),
        first(),
        map((x: any) => {
  ...
  getCurList = (search: {
    ...
    this.curriculumService
      .getAllCurriculums(search.curname, '', search.standardid, search.schoolname)
      .pipe(
  ...
  downloadStudent() {
    ...
    return this.reportService.downloadQuizzesOfClass({
      filter: filter,
      pageindex: 1,
      pagesize: 1000,
    }, this.online)
